...
public class TimeTableModel {
	 ...
	 */
	public long add(TimeTableBean bean) throws ApplicationException, DuplicateRecordsException, SQLException{
	...
	//	int pk=nextPk();
		 ...
		 if(Bean!=null)
		 {
			 throw new DuplicateRecordsException("record already existed");
		 }
	 ...
	 * @param courseName the course name
	 * @param subjectName the subject name
	 * @param exameDate the exame date
	 * @param examTime the exam time
	 * @return the time table bean
	 * @throws SQLException the SQL exception
	 */
	public  TimeTableBean findByCourseNameSubjectNameExamDateExamTime(String courseName,String subjectName,Date exameDate,String examTime) throws SQLException
	 {
		 ...
		 conn=JDBCDataSource.getConnection();
		 conn.setAutoCommit(false);
		 StringBuffer sql=new StringBuffer("select * from st_timetable where COURSE_NAME=? and SUBJECT_NAME=? and EXAM_DATE=? and EXAM_TIME=?");
	...
	public  void update(TimeTableBean bean) throws SQLException
	 {
		 ...
		 String subName=subBean.getSubjectName();
			 ...
			 try 
			 {
			  conn=JDBCDataSource.getConnection();
			  conn.setAutoCommit(false);
			  StringBuffer sql=new StringBuffer("update st_timetable set COURSE_NAME=?,COURSE_ID=?,SUBJECT_NAME=?,SUBJECT_ID=?,EXAM_DATE=?,EXAM_TIME=?,SEMESTER=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? where ID=?");
		     PreparedStatement pstmt=conn.prepareStatement(sql.toString());
	...
	public  List search(TimeTableBean Bean,int pageNo,int pageSize) throws SQLException
	 {
		 ...
		 Connection conn=null;
		StringBuffer sql=new StringBuffer("select * from st_timetable where 1=1");  ///dynamic injection return true value and at run time it will provide attribute to append with the query 
			if(Bean!=null)
			{
				if(Bean.getId()>0)
				{
